...
public class SanityController : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (stats == null)
        {
            Debug.LogError("SanityController: PlayerStatsSO belum di-assign.", this);
            return;
        }
    ...
    private void Update()
    {
        if (stats == null || IsDepleted) return;
    ...
    private void UpdateSanityOverTime()
    {
        if (isDraining)
        {
            DrainSanity(stats.SanityDrainRate * Time.deltaTime);
            return;
        }

        if (isRecovering) RecoverSanity(stats.SanityRecoveryRate * Time.deltaTime);
    }
    ...
    {
        if (stats == null || stats.MaxSanity <= 0f) return;

        float sanityRatio = currentSanity / stats.MaxSanity;

        SanityLevel newLevel;

        if (currentSanity <= 0f) newLevel = SanityLevel.Depleted;
        else if (sanityRatio <= stats.CriticalSanityThreshold) newLevel = SanityLevel.Critical;
        else if (sanityRatio <= stats.UnstableSanityThreshold) newLevel = SanityLevel.Unstable;
        else newLevel = SanityLevel.Healthy;
        ...
        if (enableDebugLog)
            Debug.Log($"[Sanity] Level: {previousLevel} → {CurrentLevel} | Sanity: {CurrentSanity:F1}/{MaxSanity:F1}", this);
    ...
    public void SetDrainActive(bool active)
    {
        ...
        if (active) isRecovering = false;

        if (enableDebugLog) Debug.Log($"[Sanity] Drain: {(active ? "ON" : "OFF")}", this);
    }
    ...
    public void SetRecoveryActive(bool active)
    {
        ...
        if (active) isDraining = false;

        if (enableDebugLog) Debug.Log($"[Sanity] Recovery: {(active ? "ON" : "OFF")}", this);
    }
    ...
    public void StopSanityChange()
    {
        ...
        if (enableDebugLog) Debug.Log("[Sanity] Sanity change stopped.", this);
    }
    ...
    public void DrainSanity(float amount)
    {
        ...
        if (enableDebugLog)
            Debug.Log($"[Sanity] Drain: {previousSanity:F1} → {currentSanity:F1} (-{amount:F1})", this);

        if (IsDepleted && enableDebugLog)
            Debug.Log("[Sanity] DEPLETED → PlayerDeathHandler akan menangani kematian.", this);
    }
    ...
    public void RecoverSanity(float amount)
    {
        ...
        float previousSanity = currentSanity;
        currentSanity = Mathf.Min(currentSanity + amount, stats.MaxSanity);
    ...
    public void SetSanity(float value)
    {
        ...
        float previousSanity = currentSanity;
        currentSanity = Mathf.Clamp(value, 0f, stats.MaxSanity);
        ...
        if (enableDebugLog) Debug.Log($"[Sanity] Set: {previousSanity:F1} → {currentSanity:F1}", this);
    }
    ...
    public void RestoreFullSanity()
    {
        ...
        if (enableDebugLog)
            Debug.Log($"[Sanity] Restored: {previousSanity:F1} → {CurrentSanity:F1}/{MaxSanity:F1}", this);
    }
}